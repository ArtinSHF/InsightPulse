...
async function callGemini(prompt, systemHint='') {
  if (!State.gemini.key) throw new Error('Please add your Gemini API key in the AI Config tab.');
  const model = State.gemini.model || 'gemini-2.5-flash';
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(State.gemini.key)}`;
  const body = {
